...
public class SwimmerIterator implements Iterator<Swimmer> {
  ...
  public SwimmerIterator(List<Swimmer> swimmers) {
    this.filteredSwimmers = this.filteredSwimmers(swimmers);
  }

  private List<Swimmer> filteredSwimmers(List<Swimmer> swimmers){
  ...
  private Boolean qualifiedFreestyleTime(Swimmer swimmer){
    for (Double swimTime: swimmer.getFreestyle50Times()){
